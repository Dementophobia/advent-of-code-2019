...
def print_message(game):
    while not len(game.output) or game.output[-1] != 10:
        run_game(game)
    while (output := read_output(game)):
        print(chr(output), end = "")

def print_camera(game):
    while len(game.output) < 2 or game.output[-2] != 10 or game.output[-1] != 10:
        run_game(game)
