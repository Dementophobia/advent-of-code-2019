...
def draw_img(field, search, steps):
    scale = 8
    
    img = Image.new('RGBA', (41*scale, 41*scale), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    
    for y in range(-19, 22):
        for x in range(-21, 20):
            rx, ry = x+21, y+19
            if field.grid[(x, y)] == "#" or field.grid[(x, y)] == "":
                draw.ellipse((rx*scale, ry*scale, rx*scale+scale-1, ry*scale+scale-1), fill=(0, 0, 0))
            elif field.grid[(x, y)] == ".":
                draw.ellipse((rx*scale, ry*scale, rx*scale+scale-1, ry*scale+scale-1), fill=(255, 255, 255))
            if search[(x, y)] == 1:
                draw.ellipse((rx*scale, ry*scale, rx*scale+scale-1, ry*scale+scale-1), fill=(0, 0, 200))
            elif search[(x, y)] == -1:
                draw.ellipse((rx*scale, ry*scale, rx*scale+scale-1, ry*scale+scale-1), fill=(0, 0, 255))
    
    file_path = path.join('images', f"field{274-steps:03}.png")
